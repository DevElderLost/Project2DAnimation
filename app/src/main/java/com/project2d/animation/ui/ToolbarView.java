...
public class ToolbarView extends View {
    ...
    public interface OnToolbarAction {
        ...
        void onToggleOnionSkin();
    }

    // Icons: Undo, Redo, sep, Tools, Brush, Timeline, sep, OnionSkin(lampu)
    private static final String[] ICONS  ={"↶","↷","|","⚒","✏","▤","|","💡"};
    private static final boolean[] IS_SEP={false,false,true,false,false,false,true,false};
    private static final int IDX_UNDO=0,IDX_REDO=1,IDX_TOOLS=3,IDX_BRUSH=4,IDX_TL=5,IDX_ONION=7;

    private final RectF[] iconRects=new RectF[8];
    ...
    private boolean toolsActive=true,brushActive=true,timelineActive=false,onionActive=false;
    ...
    private void init(){
        ...
        for(int i=0;i<8;i++) iconRects[i]=new RectF();
    ...
    public void setOnionActive(boolean a){onionActive=a;invalidate();}
    ...
    @Override protected void onDraw(Canvas canvas){
        ...
        for(int i=0;i<ICONS.length;i++){
            if(IS_SEP[i]){canvas.drawLine(x+4*density,pad,x+4*density,h-pad,pSep);iconRects[i].setEmpty();x+=14*density;continue;}
            iconRects[i].set(x,pad/2f,x+iconW,h-pad/2f);
            boolean act=(i==IDX_TOOLS&&toolsActive)||(i==IDX_BRUSH&&brushActive)||(i==IDX_TL&&timelineActive)||(i==IDX_ONION&&onionActive);
            if(act) canvas.drawRoundRect(iconRects[i],3,3,pAct);
            else if(i==hoveredIdx) canvas.drawRoundRect(iconRects[i],3,3,pHov);
            Paint.FontMetrics fm=pIcon.getFontMetrics();
            canvas.drawText(ICONS[i],iconRects[i].centerX()-pIcon.measureText(ICONS[i])/2f,iconRects[i].centerY()-(fm.ascent+fm.descent)/2f,pIcon);
            x+=iconW+pad;
        }
    }
    ...
    @Override public boolean onTouchEvent(MotionEvent e){
        ...
        switch(e.getActionMasked()){
            case MotionEvent.ACTION_DOWN: case MotionEvent.ACTION_MOVE: hoveredIdx=hit(tx,ty);invalidate();return true;
            case MotionEvent.ACTION_UP:
                int h=hit(tx,ty); hoveredIdx=-1;
                if(h>=0&&listener!=null){
                    switch(h){
                        ...
                        case IDX_TL:    timelineActive=!timelineActive; listener.onToggleTimeline(); break;
                        case IDX_ONION: onionActive=!onionActive; listener.onToggleOnionSkin(); break;
                    }
                }
    ...
    private int hit(float x,float y){for(int i=0;i<iconRects.length;i++)if(!iconRects[i].isEmpty()&&iconRects[i].contains(x,y))return i;return -1;}
}
