...
public class AnimationCanvasView extends View {
    ...
    private float gestureStartMidX  = 0f;
    private float gestureStartMidY  = 0f;
    ...
    public boolean onTouchEvent(MotionEvent e){
        ...
        switch(action){
            case MotionEvent.ACTION_DOWN:
                inTwoFinger=false; inSinglePan=true;
                lastPanX=e.getX(0); lastPanY=e.getY(0);
                handleDoubleTapCheck(e.getX(0),e.getY(0));
                break;
            case MotionEvent.ACTION_POINTER_DOWN:
                if(count>=2){ inSinglePan=false; inTwoFinger=false; startTwoFinger(e); }
                break;
            case MotionEvent.ACTION_MOVE:
                if(count>=2){
                    if(!inTwoFinger){ inSinglePan=false; startTwoFinger(e); }
                    else{ updateTwoFinger(e); }
                } else if(inSinglePan&&!inTwoFinger){
                    float dx=e.getX(0)-lastPanX, dy=e.getY(0)-lastPanY;
                    matrix.postTranslate(dx,dy);
                    lastPanX=e.getX(0); lastPanY=e.getY(0); invalidate();
                }
                break;
            case MotionEvent.ACTION_POINTER_UP:
                inTwoFinger=false;
                if(count==2){ int k=(e.getActionIndex()==0)?1:0; lastPanX=e.getX(k); lastPanY=e.getY(k); inSinglePan=true; }
                trySnapAngle(); break;
            case MotionEvent.ACTION_UP:
                inSinglePan=false; inTwoFinger=false; trySnapAngle(); break;
            case MotionEvent.ACTION_CANCEL:
                inSinglePan=false; inTwoFinger=false; break;
        }
        return true;
    }
    ...
    private void updateTwoFinger(MotionEvent e){
        float nd=span(e), na=fingerAngle(e), nmx=midX(e), nmy=midY(e);
        if(gestureStartDist<1f) return;
        float base=mScale(matrixAtGestureStart);
        float tgt=Math.max(MIN_SCALE,Math.min(MAX_SCALE,base*(nd/gestureStartDist)));
        float sd=tgt/base, rd=na-gestureStartAngle;
        float pdx=nmx-gestureStartMidX, pdy=nmy-gestureStartMidY;
        matrix.set(matrixAtGestureStart);
        matrix.postTranslate(pdx,pdy);
        matrix.postRotate(rd,nmx,nmy);
        matrix.postScale(sd,sd,nmx,nmy);
    ...
    private void trySnapAngle(){
        ...
        for(float c:new float[]{0f,90f,180f,270f,360f}){
            if(Math.abs(a-c)<=SNAP_THRESH){ float[] pv=canvasCenter(); animSnap((c%360f)-a,pv[0],pv[1]); return; }
        }
    }
    ...
    private void animSnap(float delta,float px,float py){
        ...
        float[] prev={0f};
        ValueAnimator a=ValueAnimator.ofFloat(0f,delta); a.setDuration(160); a.setInterpolator(new DecelerateInterpolator());
    ...
    private void handleDoubleTapCheck(float x,float y){
    ...
    private void animReset(){
        ...
        a.addUpdateListener(va->{ float t=(float)va.getAnimatedValue(),iv[]=new float[9]; for(int i=0;i<9;i++) iv[i]=sv[i]+(tv[i]-sv[i])*t; matrix.setValues(iv); currentScale=sv[0]+(s-sv[0])*t; currentAngle=0f; invalidate(); });
        a.start();
    }
    ...
    private void drawHud(Canvas canvas,float vw,float vh){
        ...
        if(showAngleHint&&System.currentTimeMillis()<angleHintUntil){
            ...
            hudPill(canvas,String.format("%.1f\u00b0",disp),vw-8*d,vh-30*d,d); invalidate();
        }
    }
